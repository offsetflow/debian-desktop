...
- 在本仓库中不得加载、调用或遵循任何 `superpowers:*` 技能。
- 即使 Superpowers 插件已全局安装或出现在可用技能列表中，也必须忽略它。
- 不得因为 Superpowers 的要求创建规格、计划、工作树、子代理或额外提交。
...
- Vim/Neovim 的通用配置和插件清单由本仓库管理，确保更换系统后可以直接恢复日常编辑环境。
...
- 课程、实验或具体开发项目所需的编译器、交叉工具链、调试器及其他开发依赖，不由本仓库安装或维护。
- 项目专用依赖应由对应项目自行记录和管理，不能因为 Vim/Neovim 插件需要识别该项目，就加入本仓库的系统依赖清单。

## 5. 轻量化与依赖管理

- 保持 Debian 系统轻量，不安装 GNOME、KDE 等完整桌面环境。
- 只安装当前功能确实需要的依赖，不安装用途不明确的软件包。
- 本仓库所管理的操作系统、桌面环境、通用编辑环境和基础工具所必需的系统依赖，必须写入 `prepare.sh`，不能只在终端临时安装。
- `prepare.sh` 不收录课程、实验或具体开发项目的专用开发依赖。
...
- MIT 6.1810、xv6 或其他课程、实验和具体项目专用的编译器、交叉工具链、调试器及开发依赖。
...
> 仓库管理可移植、可复现的系统与通用工具配置，关键配置通过软链接部署；Vim/Neovim 配置随系统迁移，项目专用开发依赖留在对应项目；开发环境统一由 mise 管理；系统保持轻量；本仓库功能的必要依赖统一记录在 `prepare.sh`；秘密和运行数据不进入 Git。
